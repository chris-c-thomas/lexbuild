...
- **`generate-highlights.ts`** uses Shiki dual themes (`github-light-default` + `github-dark-default`). Processes 300k+ files by forking child processes in chunks of 10k — each child gets its own Shiki instance and exits when done, fully releasing memory. This prevents OOM (Shiki's internal grammar caches leak ~4GB over 260k+ files in a single process). To change themes, update both this script and `src/lib/shiki.ts`, delete existing `.highlighted.html` files, then re-run.
...
- **Download filenames** use hierarchical naming with source prefix: `usc-title-01-chapter-01-section-1.md`, `ecfr-title-17-chapter-IV-part-240-section-240.10b-5.md`. Built from the slug segments in each catch-all route.
- **Download mechanism** uses `<script type="text/plain">` + Blob URL (not `data:` URI or `<template>`). Do NOT use `<template>` — it parses HTML-like content in Markdown, corrupting the download. `<script type="text/plain">` preserves content exactly. A top-level `<script>` creates a Blob URL on click.
