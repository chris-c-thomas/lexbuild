...
- **Scalar's `darkMode` config prop only applies on init**: `useColorMode` reads `initialColorMode` once during Vue setup. Subsequent prop changes via `updateConfiguration()` do NOT toggle the theme. Workaround: directly toggle `.dark-mode`/`.light-mode` on `document.body` via a React `useEffect`.
