...
- **Link Resolver** -- Register/resolve/fallback system for cross-references. Handles both USC (`/us/usc/`) and CFR (`/us/cfr/`) identifier schemes, producing relative Markdown links within the converted corpus or falling back to external URLs (OLRC for USC, ecfr.gov for CFR).
