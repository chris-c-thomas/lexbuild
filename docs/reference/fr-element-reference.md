# FR Element Reference

Element classification for Federal Register XML. The FR XML format is GPO/SGML-derived with no namespace, sharing many inline elements with eCFR but using a flat, document-centric structure rather than hierarchical DIVs.

The authoritative source for this classification is `packages/fr/src/fr-elements.ts`.
...
Group documents within daily issues. Pass-through in the builder.
...
Appear within `SUPLINF` in rules and proposed rules.
