...
- **FR date filters are two-stage**: `discoverXmlFiles` filters at month granularity (per-doc paths only encode `YYYY/MM/`, so path-inferred dates are `YYYY-MM-01`); `convertFrDocuments` applies a precise day-level filter using `publication_date` from each JSON sidecar. A single-stage day-level filter excludes every file in the `from`-month on mid-month ranges (regression: see `converter.test.ts` "regression: --from mid-month").
